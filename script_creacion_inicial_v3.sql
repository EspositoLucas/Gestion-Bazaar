...
IF EXISTS (SELECT name FROM sys.objects WHERE name = 'GetTipoVariante')
...
create function UBUNTEAM_THE_SQL.GetVenta (
	...
	@Id_medio_envio_por_localidad int ,
	@venta_envio_precio decimal(18,2),
	@venta_canal_costo decimal(18,2) ,
	@venta_medio_de_pago_costo decimal(18,2)
)
...
begin
	...
	declare @idVenta int = (select Venta.Id from UBUNTEAM_THE_SQL.Venta
									...
									and Id_medio_envio_por_localidad = @Id_medio_envio_por_localidad
...
begin 
	...
	insert into UBUNTEAM_THE_SQL.Venta(venta_codigo,Id_prod_por_var,venta_fecha,Id_cliente,venta_total,Id_canal,Id_medio_de_pago,Id_medio_envio_por_localidad,venta_canal_costo,venta_medio_de_pago_costo)
...
begin
	insert into UBUNTEAM_THE_SQL.ProductoPorVariante(Id_producto,Id_variante,precio_compra,precio_venta,prod_var_cantidad_compra,prod_var_cantidad_venta)
		select distinct UBUNTEAM_THE_SQL.GetProducto(M.PRODUCTO_CODIGO,M.PRODUCTO_DESCRIPCION,M.PRODUCTO_MARCA,M.PRODUCTO_MATERIAL,M.PRODUCTO_NOMBRE),
		...
		from gd_esquema.Maestra as M
		where M.PRODUCTO_TIPO_VARIANTE is not null and M.PRODUCTO_VARIANTE_CODIGO is not null and M.PRODUCTO_CODIGO is not null and M.COMPRA_PRODUCTO_PRECIO is not null and M.VENTA_PRODUCTO_PRECIO is not null and M.COMPRA_PRODUCTO_CANTIDAD is not null and M.VENTA_PRODUCTO_CANTIDAD is not null
end
go
...
create procedure UBUNTEAM_THE_SQL.Migrar_DescuentoPorCompra
 ...
 begin 	insert into UBUNTEAM_THE_SQL.DescuentoPorCompra(descuento_compra_codigo, Id_compra,desc_compra_valor)
		select distinct M.DESCUENTO_COMPRA_CODIGO, UBUNTEAM_THE_SQL.GetCompra( M.COMPRA_NUMERO,
		UBUNTEAM_THE_SQL.GetProductoPorVariante(UBUNTEAM_THE_SQL.GetProducto(M.PRODUCTO_CODIGO,M.PRODUCTO_DESCRIPCION,M.PRODUCTO_MARCA,M.PRODUCTO_MATERIAL,M.PRODUCTO_NOMBRE),
		...
		UBUNTEAM_THE_SQL.GetLocalidad(M.PROVEEDOR_LOCALIDAD,UBUNTEAM_THE_SQL.GetProvincia(M.PROVEEDOR_PROVINCIA),M.PROVEEDOR_CODIGO_POSTAL),M.PROVEEDOR_MAIL),M.COMPRA_TOTAL,
						UBUNTEAM_THE_SQL.GetMedioDePago( M.COMPRA_MEDIO_PAGO,null)) ,M.DESCUENTO_COMPRA_VALOR
...
--select distinct UBUNTEAM_THE_SQL.GetProducto(M.PRODUCTO_CODIGO,M.PRODUCTO_DESCRIPCION,M.PRODUCTO_MARCA,M.PRODUCTO_MATERIAL,M.PRODUCTO_NOMBRE) from gd_esquema.Maestra M
...
create procedure UBUNTEAM_THE_SQL.Migrar_DescuentoPorVenta
 ...
 begin 	insert into UBUNTEAM_THE_SQL.DescuentoPorVenta(Id_venta,desc_venta_importe,Id_tipo_descuento)
		...
		UBUNTEAM_THE_SQL.GetProductoPorVariante(
		UBUNTEAM_THE_SQL.GetProducto(M.PRODUCTO_CODIGO,M.PRODUCTO_DESCRIPCION,M.PRODUCTO_MARCA,M.PRODUCTO_MATERIAL,M.PRODUCTO_NOMBRE),
		UBUNTEAM_THE_SQL.GetVariante(M.PRODUCTO_VARIANTE_CODIGO,M.PRODUCTO_VARIANTE,UBUNTEAM_THE_SQL.GetTipoVariante(M.PRODUCTO_TIPO_VARIANTE)),
		M.COMPRA_PRODUCTO_PRECIO,M.VENTA_PRODUCTO_PRECIO,M.COMPRA_PRODUCTO_CANTIDAD,M.VENTA_PRODUCTO_CANTIDAD
		),
		M.VENTA_FECHA,  
		UBUNTEAM_THE_SQL.GetCliente(M.CLIENTE_NOMBRE,M.CLIENTE_DNI,
		UBUNTEAM_THE_SQL.GetLocalidad(M.CLIENTE_LOCALIDAD,UBUNTEAM_THE_SQL.GetProvincia(M.CLIENTE_PROVINCIA),M.CLIENTE_CODIGO_POSTAL),
		M.CLIENTE_TELEFONO,M.CLIENTE_APELLIDO,M.CLIENTE_FECHA_NAC,M.CLIENTE_DIRECCION,M.CLIENTE_MAIL),
		M.VENTA_TOTAL,
		UBUNTEAM_THE_SQL.GetCanal(M.VENTA_CANAL,null),
		UBUNTEAM_THE_SQL.GetMedioDePago(M.VENTA_MEDIO_PAGO,null),
		UBUNTEAM_THE_SQL.GetMedioEnvioPorLocalidad(
		UBUNTEAM_THE_SQL.GetMedioEnvio(M.VENTA_MEDIO_ENVIO),
		UBUNTEAM_THE_SQL.GetLocalidad(M.CLIENTE_LOCALIDAD,UBUNTEAM_THE_SQL.GetProvincia(M.CLIENTE_PROVINCIA),M.CLIENTE_CODIGO_POSTAL)
		,null), M.VENTA_ENVIO_PRECIO,
		M.VENTA_CANAL_COSTO,M.VENTA_MEDIO_PAGO_COSTO),
		--esto es del insert into
...
as
begin
	insert into UBUNTEAM_THE_SQL.VentaPorCupon(Id_venta,venta_cupon_importe)
